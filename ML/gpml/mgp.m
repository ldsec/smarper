function [varargout] = mgp(hyp, inf, mean, cov, lik, x, y, xs, ys)
...
if nargin==7                                     % if no test cases are provided
  ...
  while nact<ns               % process minibatches of test cases to save memory
	...
	assert( size(ms,1) ==n_test )
    N = size(alpha,2);  % number of alphas (usually 1; more in case of sampling)

	if strcmp(lik_name,'likSoftmax')
		assert(N==1);
		Fmu=zeros(C*n_test,1);%C*n by 1
		ymu=zeros(ns,1);
		for j=1:C
			idx1=(j-1)*n_test+1:j*n_test;
			idx2=(j-1)*n_train+1:j*n_train;
			assert( length(idx2)==n_train );
			Fmu(idx1,:)=ms+Ks'*alpha(idx2,:);
		end
	else
    ...
    if Lchol    % L contains chol decomp => use Cholesky parameters (alpha,sW,L)
      ...
      fs2(id) = kss - sum(V.*V,1)';                       % predictive variances
		...
		if strcmp(lik_name,'likSoftmax')
		  R=sW;
		  Fs2= zeros(C*n_test, C*n_test);  
		  Ks_C=zeros(C*n_train,C*n_test);
		  %Ks %n_train by n_test
		  for j=1:C
			  idx1=(j-1)*n_test+1:j*n_test;
			  idx2=(j-1)*n_train+1:j*n_train;
			  Fs2(idx1,idx1)=diag(kss);
			  Ks_C(idx2,idx1)=Ks;
		  end
		  T = R'\(L'*Ks_C);
		  Fs2=Fs2-T'*T;
		  assert( all( diag(Fs2)>0)  )
	  else
    ...
    if nargin<9
	  ...
	  if ~strcmp(lik_name,'likSoftmax')
		  ...
		  Lp = feval(lik{:},hyp.lik,Ys,Fmu,Fs2);
	  end
    end
