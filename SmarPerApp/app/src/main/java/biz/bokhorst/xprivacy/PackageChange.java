				...
				if (intent.getAction().equals(Intent.ACTION_PACKAGE_ADDED)) {
					...
					if (appInfo.getPackageName().size() == 1) {
						if (replacing)
							...
							//ID: Disabled, so no new apps will be monitored
							//PrivacyManager.applyTemplate(uid, "Template1", null, true, true, false);
					...
					boolean notify = PrivacyManager.getSettingBool(userId, PrivacyManager.cSettingNotify, false);         //JS: Setting the default value of notify to false

					if (notify){
						//notify = PrivacyManager.getSettingBool(-uid, PrivacyManager.cSettingNotify, true);
						notify = PrivacyManager.getSettingBool(-uid, PrivacyManager.cSettingNotify, false);         //JS: Setting the default value of notify to false
					}
					if (/*!replacing || */notify) {         											//JS: build notification only when notify is true (which is never the case)
						...
						notificationBuilder.setSmallIcon(R.mipmap.ic_launcher);		//JS: Change notification icon to SmarPer icon
