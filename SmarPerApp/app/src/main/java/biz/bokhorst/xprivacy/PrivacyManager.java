...
public class PrivacyManager {
	...
	static int previewFramesToSave = 15; ///KO
	static int previewFramesSaved = 0; //KO: for debugging preview
	static int R = 6371; //KO: Earth radius in km. Perhaps move to SmarPerUtil
	static double d = 10.0; //KO: Length of side of obfuscated square for location, in km

	private static boolean cameraOpenStatus = false; //KO: Store most recent camera status, only update service if there was a change
	private static boolean typingStatus = false; //KO: Store most recent typing status, only update service if there was a change
	...
	public final static String cSettingSmarperPolicyCreated = "SmarperPolicyCreated"; //ID
	public final static String cSettingSmarperPolicyApplied = "SmarPerPolicyApplied"; //ID
	public final static String cSettingSmarperPolicyName = "SmarperPolicy"; //ID
	// Special value names
	...
	//ID: List of monitored apps
	public final static List cMonitoredApps = Arrays.asList(
			"com.twitter.android",
			"com.whatsapp",
			"com.skype.raider",
			"kik.android",
	...
	//KO: Usage is always FALSE (so this method does not prompt or record usage data).
	...
	//KO: Our version of this method with time
	public static boolean getRestrictionTime(final XHook hook, int uid, String restrictionName, String methodName,
			String secret, long time) {
		return getRestrictionExtraTime(hook, uid, restrictionName, methodName, null, null, secret, time);
	}
	
	//KO: For "Obfuscate"
	public static int getPRestrictionTime(final XHook hook, int uid, String restrictionName, String methodName,
			String secret, long time) {
		return getPRestrictionExtraTime(hook, uid, restrictionName, methodName, null, null, secret, time);
	}
	...
	//KO: Our version of this method with time
	public static boolean getRestrictionExtraTime(final XHook hook, int uid, String restrictionName, String methodName,
			String extra, String secret, long time) {
		return getRestrictionExtraTime(hook, uid, restrictionName, methodName, extra, null, secret, time);
	}
	
	//KO: For obfuscate
	public static int getPRestrictionExtraTime(final XHook hook, int uid, String restrictionName, String methodName,
			String extra, String secret, long time) {
		return getPRestrictionExtraTime(hook, uid, restrictionName, methodName, extra, null, secret, time);
	}
	...
	public static boolean getRestrictionExtra(final XHook hook, int uid, String restrictionName, String methodName,
		...
		// Check cache
		//KO: Disable caching here for now
		boolean cached = false;
	...
	//KO: Our version of this method
		public static boolean getRestrictionExtraTime(final XHook hook, int uid, String restrictionName, String methodName,
				String extra, String value, String secret, long time) {
			long start = System.currentTimeMillis();
			PRestriction result = new PRestriction(uid, restrictionName, methodName, false, true);
			...
			if (!cached)
				try {
					...
					query.extra = extra;
					query.value = value;
					PRestriction restriction = PrivacyService.getRestrictionProxyTime(query, true, secret, time);
					result.restricted = restriction.restricted;
					if (restriction.debug)
						Util.logStack(null, Log.WARN);
		...
		//KO: Version for including "Obfuscate" as an option
				public static int getPRestrictionExtraTime(final XHook hook, int uid, String restrictionName, String methodName,
						String extra, String value, String secret, long time) {
					long start = System.currentTimeMillis();
					PRestriction result = new PRestriction(uid, restrictionName, methodName, false, true);
					...
					synchronized (mRestrictionCache) {
						if (mRestrictionCache.containsKey(key)) {
							...
							if (!entry.isExpired()) {
								cached = true;
								result.restricted = entry.restricted;
								result.restrictState = entry.restrictState;
								result.asked = entry.asked;
							}
						}
					}
					...
					if (!cached)
						try {
							...
							PRestriction restriction = PrivacyService.getRestrictionProxyTime(query, true, secret, time);
							result.restricted = restriction.restricted;
							result.restrictState = restriction.restrictState; //KO: Copy this over

							//KO: TODO: wtf, can we just set result = restriction returned from PrivacyService?
							...
							// Add to cache
							if (result.time >= 0) {
								key.restrictState = result.restrictState;
								key.restricted = result.restricted;
								key.asked = result.asked;
	...
	public static List<PRestriction> getRestrictionList(int uid, String restrictionName) { //KO: Used for showing the checkbox state and filtering the app list
		checkCaller();
	...
	//KO: For reading from our table
	public static List<PRestriction> getFullUsageList(int uid, String restrictionName) {
		checkCaller();

		List<PRestriction> listUsage = new ArrayList<PRestriction>();
		try {
			listUsage.addAll(PrivacyService.getClient().getFullUsageList(uid,
					restrictionName == null ? "" : restrictionName));
		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "getFullUsageList");
			Util.bug(null, ex);
		}
		Collections.sort(listUsage, new ParcelableRestrictionCompare());
		return listUsage;
	...
	//JS: For fetching app icons from database
	public static List<appPackNameAndIcon> getAppIcons() {
		checkCaller();
		List<appPackNameAndIcon> listOfIcons = new ArrayList<appPackNameAndIcon>();
		try {
			listOfIcons = PrivacyService.getClient().getAppIcons();
		} catch (Throwable ex) {Util.log(null, Log.ERROR, "getAppIcons");
			Util.bug(null, ex);
		}
		return listOfIcons;
	}

	//JS: For Getting the filtered data, as requested by the user
	public static List<PRestriction> GetFilteredData(int[] decision, int[] decisiontype, int time, List<String> MethCat, List<String> AppList) {
		checkCaller();

		List<PRestriction> listUsage = new ArrayList<PRestriction>();
		try {
			listUsage.addAll(PrivacyService.getClient().GetFilteredData(decision, decisiontype, time, MethCat, AppList));
		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "GetFilteredData");
			Util.bug(null, ex);
		}
		Collections.sort(listUsage, new ParcelableRestrictionCompare());
		return listUsage;
	...
	//JS: For Connecting to Server and Uploading Data
	public static int ConnectToServerandSendData(String url) {
		checkCaller();
		int NumberofRowsReceived = -5;              //JS: To Know What the Server returned: -1 (error) or a positive number (success)
		try {
			NumberofRowsReceived = PrivacyService.getClient().ConnectToServerandSendData(url);
		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "ConnectToServerandSendData");
			Util.bug(null, ex);
		}
		return NumberofRowsReceived;
	}

	//JS: For Clearing the Cache
	public static int ClearCacheOfSelectedApp(int uid) {
		checkCaller();
		int numberOfRemovedEntries = 0;
		try {
			numberOfRemovedEntries = PrivacyService.getClient().ClearCacheOfSelectedApp(uid);
		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "ClearCacheOfSelectedApp");
			Util.bug(null, ex);
		}
		return numberOfRemovedEntries;
	}

	//JS: For Updating the Database on User Decision Change
	public static void UpdateDBwithNewDecision(int radioId, int idValue) {
		checkCaller();
		try {
			PrivacyService.getClient().UpdateDBwithNewDecision(radioId, idValue);
		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "UpdateDBwithNewDecision");
			Util.bug(null, ex);
		}
	}

	//JS: For Fetching the Information to be Dispayed when a decision is pressed in the usage view
	public static List<String> PermissionInfoOnClick(int idValue) {
		checkCaller();
		List<String> listInfo = new ArrayList<String>();
		try {
			listInfo = PrivacyService.getClient().PermissionInfoOnClick(idValue);

		} catch (Throwable ex) {
			Util.log(null, Log.ERROR, "PermissionInfoOnClick");
			Util.bug(null, ex);
		}
		return listInfo;
	}
	...
	//KO: New method for removing accuracy from location
	public static Location getObfuscatedLocation(Location location){

		//Kevin's new obfuscation code
		double lat = location.getLatitude();
		double lng = location.getLongitude();

		double dlat = (d * 360) / (R * 2 * Math.PI);
		double lat_ = (Math.floor(lat/dlat) + .5) * dlat;
		double dlng = (d * 360) / (R * 2 * Math.PI * Math.cos(lat_ * (Math.PI/180.0)));
		double lng_ = (Math.floor(lng/dlng) + .5) * dlng;

		//Accuracy
		//Low : radius > 500 m
		location.setAccuracy(Criteria.ACCURACY_LOW);
		...
		location.setLongitude(lng_);

		return location;
		
	}
	
		//KO: Obfuscate preview frames
		public static byte[] getObfuscatedPreviewFrame(byte[] picture, Camera camera){
			...
			try{
				...
				//If camera was already released, we will get a runtime exception
				return picture;
			}
			...
			YuvImage yuv_img = new YuvImage(picture,previewFormat,s.width,s.height,null);
			ByteArrayOutputStream ba = new ByteArrayOutputStream();
			
			if (!yuv_img.compressToJpeg(new Rect(0, 0, yuv_img.getWidth(), yuv_img.getHeight()), 100, ba)){
				Log.e("Smarper-Error", "Could not compress to JPEG...");
				return picture;
			}
			
			Options opts = new BitmapFactory.Options();
			opts.inMutable = true;
			byte[] jpg = ba.toByteArray();
			
			Bitmap obfus = BitmapFactory.decodeByteArray(jpg, 0, jpg.length, opts);
			if (obfus == null){
				Log.e("Smarper-Error", "Could not import image data... ");
				return picture;
			}
			else{
			
				int originalWidth = obfus.getWidth();
				int originalHeight = obfus.getHeight();
				
				Bitmap small = Bitmap.createScaledBitmap(obfus, originalWidth/14, originalHeight/14, false);
				Bitmap dest = Bitmap.createScaledBitmap(small, originalWidth, originalHeight, true);
				
			    byte [] modified_yuv = new byte[s.width*s.height*3/2];
			    int [] pixels = new int[s.width * s.height];
			    dest.getPixels(pixels, 0, s.width, 0, 0, s.width, s.height);
			    encodeYUV420SP(modified_yuv, pixels, s.width, s.height);
			    
			    //Make sure it looks OK
			   /* if (previewFramesSaved < previewFramesToSave){
			    FileOutputStream out = null;
			    try {
			    	
			    	Log.d("Smarper-Debug", "Preview size is (w,h): " + s.width + ", " + s.height);
			    	Log.d("Smarper-Debug", "YUV size is (w,h): " + yuv_img.getWidth() + ", " + yuv_img.getHeight());
			    	Log.d("Smarper-Debug", "Orignial bitmap size is (w,h): " + obfus.getWidth() + ", " + obfus.getHeight());
			    	Log.d("Smarper-Debug", "Obfuscated bitmap size is (w,h): " + dest.getWidth() + ", " + dest.getHeight());
			    	
			        out = new FileOutputStream(Environment.getExternalStorageDirectory().getPath() + "/yuv_encoded" + previewFramesSaved +".jpg");
			        
			        YuvImage finalFrame = new YuvImage(modified_yuv,previewFormat,s.width,s.height,null);
			        ByteArrayOutputStream ba2 = new ByteArrayOutputStream();
			        finalFrame.compressToJpeg(new Rect(0, 0, finalFrame.getWidth(), finalFrame.getHeight()), 100, ba2);
			        out.write(ba2.toByteArray());
		...
		//KO: Deface preview frames
		public static byte[] getDefacedPreviewFrame(byte[] picture, Camera camera){
			...
			if (obfus == null){
				...
				obfus.eraseColor(Color.BLACK);
				
			    byte [] modified_yuv = new byte[s.width*s.height*3/2];
			    int [] pixels = new int[s.width * s.height];
			    obfus.getPixels(pixels, 0, s.width, 0, 0, s.width, s.height);
			    encodeYUV420SP(modified_yuv, pixels, s.width, s.height);
	...
	//KO: Obfuscate photos from camera
	public static byte[] getObfuscatedPictureTaken(byte[] picture){
		
		//Scale the image down and then back up
		Options opts = new BitmapFactory.Options();
		opts.inMutable = true;
		Bitmap obfus = BitmapFactory.decodeByteArray(picture, 0, picture.length, opts);
		if (obfus == null){
			Log.e("Smarper-Error", "Could not import image data... ");
			return picture;
		}
			...
			Bitmap small = Bitmap.createScaledBitmap(obfus, originalWidth/20, originalHeight/20, false);
			Bitmap dest = Bitmap.createScaledBitmap(small, originalWidth, originalHeight, true);
			ByteArrayOutputStream bas = new ByteArrayOutputStream();
			
			if(dest.compress(Bitmap.CompressFormat.JPEG, 50, bas)){
				picture = bas.toByteArray();
				}
			else{
			Log.e("Smarper-Error", "Failed to compress image...");
			}
	...
	//KO: Deface photos from camera
		public static byte[] getDefacedPictureTaken(byte[] picture){
			
			Options opts = new BitmapFactory.Options();
			opts.inMutable = true;
			Bitmap orig = BitmapFactory.decodeByteArray(picture, 0, picture.length, opts);
			if (orig == null){
				Log.e("Smarper-Error", "Could not import image data... ");
				return picture;
			}
				...
				orig.eraseColor(Color.BLACK);
				
				//KO: Start write text on image
				Canvas newCanvas = new Canvas(orig);   
				Paint textPaint = new Paint(); 
				textPaint.setColor(Color.WHITE);
				textPaint.setTextSize(150);
				Rect rectText = new Rect();
				String captionText = "Pictures not allowed!";
			    textPaint.getTextBounds(captionText, 0, captionText.length(), rectText); //Get size of text so we can center it
				newCanvas.drawText(captionText, orig.getWidth()/2 - Math.abs(rectText.width()/2), orig.getHeight()/2, textPaint);
				//KO: End write text on image
				
				
				
				ByteArrayOutputStream bas = new ByteArrayOutputStream();
				
				if(orig.compress(Bitmap.CompressFormat.JPEG, 50, bas)){
					picture = bas.toByteArray();
					}
				else{
				Log.e("Smarper-Error", "Failed to compress image...");
	...
	public static Location getDefacedLocation(int uid, Location location) {
		...
		//Log.d("Smarper-Debug", uid + " Defaced location: " + location.getLatitude() + ", " + location.getLongitude());
		return location;
	}
	...
	//KO: Send the intercepted camera status to the PrivacyService.
	public static void updateCameraStatus(boolean newStatus) {

		if (newStatus != cameraOpenStatus) { //Update only if there was a change.
			try {
				cameraOpenStatus = newStatus;
				PrivacyService.getClient().updateCameraStatus(newStatus);
				//Log.d("Smarper-debug", "Updating PrivacyService with new camera status");
			} catch (Exception e) {
				Log.e("Smarper-Error", "Exception when trying to update camera status: " + e.getMessage());
				e.printStackTrace();
	...
	//KO: Send the intercepted typing status to the PrivacyService.
	public static void updateTypingStatus(boolean newStatus) {

		if (newStatus != typingStatus) { //Update only if there was a change
			try {
				typingStatus = newStatus;
				PrivacyService.getClient().updateTypingStatus(newStatus);
				//Log.d("Smarper-debug", "Updating PrivacyService with new typing status");
			} catch (Exception e) {
				Log.e("Smarper-Error", "Exception when trying to update typing status: " + e.getMessage());
				e.printStackTrace();
	...
	//KO: Got back button press which may close the keyboard if it is open.
	public static void toggleTypingStatusIfTyping(){
