...
public class XLocationManager extends XHook {
	...
	private static final Map<Object, Object> mMapProxyObfus = new WeakHashMap<Object, Object>(); //KO
	...
	protected void before(XParam param) throws Throwable {
		...
		case Srv_addGpsNavigationMessageListener:
			if (getRestrictState(param)==2)//if (isRestricted(param)) //KO
				param.setResult(false);
			break;

		case Srv_removeGpsStatusListener:
			if (getRestrictState(param, PrivacyManager.cLocation, "Srv_addGpsStatusListener") ==2)//if (isRestricted(param, PrivacyManager.cLocation, "Srv_addGpsStatusListener")) //KO
				param.setResult(null);
			break;
		...
		case removeUpdates:
			int rs = getRestrictState(param, PrivacyManager.cLocation, "requestLocationUpdates");
			switch (rs){
			case 1:
				unproxyLocationObfuscater(param, 0);
				break;
			case 2:
				unproxyLocationListener(param, 0, true); //isRestricted(param, PrivacyManager.cLocation, "requestLocationUpdates"))
				break;
			}
				
			break;

			//KO: Add obfuscation case here
		case requestLocationUpdates:
			if (param.args.length > 0 && param.args[0] instanceof String) {
				int restrictState = getRestrictState(param, (String) param.args[0]);
				
				if (restrictState == 1 || restrictState ==2){ 
				switch (getRestrictState(param, (String) param.args[0])){ //if (isRestrictedExtra(param, (String) param.args[0]))
				case 1:
					proxyLocationObfuscater(param, 3, LocationListener.class, true);
					break;
				case 2:
		...
		case Srv_removeUpdates:
			int restrictState = getRestrictState(param, PrivacyManager.cLocation, "Srv_requestLocationUpdates");
				...
				switch (getRestrictState(param)) {//if (isRestricted(param))
					case 1:
						if (param.args[1] != null) // ILocationListener
							proxyLocationObfuscater(param, 1, Class.forName("android.location.ILocationListener"), false);
		...
		case requestSingleUpdate:
			if (param.args.length > 0 && param.args[0] instanceof String) {
				...
				switch (getRestrictState(param, (String) param.args[0])){//if (isRestrictedExtra(param, (String) param.args[0]))
				case 1:
					proxyLocationObfuscater(param, 1, LocationListener.class, true);
					break;
				case 2:
	...
	protected void after(XParam param) throws Throwable {
		...
		//KO: We can't really obfuscate this without breaking the app, it will think location services are off
		case isProviderEnabled:
		case Srv_isProviderEnabled:
			if (param.args.length > 0) {
				String provider = (String) param.args[0];
				//int restrictState = getRestrictStateExtra(param, provider);
				/*switch (restrictState){				//if (isRestrictedExtra(param, provider)){
				case 2:
					Log.d("Smarper-Dialogs", mMethod + " is restricted!");
					param.setResult(false);
				}*/
		...
		//KO: Same case here, needs to be enabled
		case getProviders:
		case getAllProviders:
		case Srv_getAllProviders:
		case Srv_getProviders:
			//int restrictState = getRestrictState(param);
			//if (restrictState == 2)//if (isRestricted(param)) //KO
		...
		case Srv_getBestProvider:
			...
			//KO: Add the "obfuscate" case for this method
		case getLastKnownLocation:
			if (param.args.length > 0 && param.getResult() instanceof Location) {
				String provider = (String) param.args[0];
				Location location = (Location) param.getResult();
				switch(getRestrictStateExtra(param, provider)){
				case 1:					
					param.setResult(PrivacyManager.getObfuscatedLocation(location)); //KO: Work is done in here, our new method
					break;
			...
			//KO: Add "obfuscate" case here too 
		case Srv_getLastLocation:
		...
		if (param.args.length > arg)
			if (param.args[arg] instanceof PendingIntent)
				...
				if (client) {
					...
					InvocationHandler ih = new OnLocationChangedHandler(Binder.getCallingUid(), param.args[arg], 2); // Deny
	...
	//KO
	private void proxyLocationObfuscater(XParam param, int arg, Class<?> interfaze, boolean client) throws Throwable {
		if (param.args.length > arg)
			if (param.args[arg] instanceof PendingIntent)
				param.setResult(null);
				...
				// Create proxy
				ClassLoader cl = param.thisObject.getClass().getClassLoader();
				InvocationHandler ih = new OnLocationChangedHandler(Binder.getCallingUid(), param.args[arg], 1); //Obfuscate
				Object proxy = Proxy.newProxyInstance(cl, new Class<?>[] { interfaze }, ih);
				...
				// Use proxy
				synchronized (mMapProxyObfus) {
					mMapProxyObfus.put(key, proxy);
				}
				param.args[arg] = proxy;
			}
	}
	...
	private void unproxyLocationObfuscater(XParam param, int arg) {
		if (param.args.length > arg)
			if (param.args[arg] instanceof PendingIntent)
				param.setResult(null);
				...
				synchronized (mMapProxyObfus) {
					if (mMapProxyObfus.containsKey(key))
						param.args[arg] = mMapProxyObfus.get(key);
				}
			}
	}
