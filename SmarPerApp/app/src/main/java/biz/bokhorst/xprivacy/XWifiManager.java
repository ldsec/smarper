...
public class XWifiManager extends XHook {
	...
	@Override
	//@SuppressWarnings("rawtypes") //KO: unnecessary
	protected void after(XParam param) throws Throwable {
		switch (mMethod) {
		case Srv_getBatchedScanResults:
			if (param.getResult() != null) 
				switch (getRestrictState(param)){//if (isRestricted(param)) //KO: Do the same thing for obfuscate
				case 1:
				case 2:
		...
		case Srv_getScanResults:
			if (param.getResult() != null)
					switch (getRestrictState(param)){//replace if (isRestricted(param) //KO: Do the same thing for obfuscate
							case 1:
							case 2:
								param.setResult(new ArrayList<ScanResult>());	
								break;
