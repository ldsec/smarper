...
public class XActivityRecognitionApi extends XHook {
	...
	protected void before(XParam param) throws Throwable {
		...
		case removeActivityUpdates:
			int restrictState = getRestrictState(param, PrivacyManager.cLocation, "GMS5.requestActivityUpdates");
			if (restrictState ==1 | restrictState == 2)//if (isRestricted(param, PrivacyManager.cLocation, "GMS5.requestActivityUpdates"))
		...
		case requestActivityUpdates:
			int restrictState1 = getRestrictState(param);
			if (restrictState1 ==1 | restrictState1 == 2)//(isRestricted(param))
				param.setResult(XGoogleApiClient.getPendingResult(param.thisObject.getClass().getClassLoader()));
			break;
		}
	}
