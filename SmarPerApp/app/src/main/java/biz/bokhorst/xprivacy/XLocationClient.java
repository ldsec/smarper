...
public class XLocationClient extends XHook {
	...
	private static final Map<Object, Object> mMapProxy2 = new WeakHashMap<Object, Object>();
	...
	protected void before(XParam param) throws Throwable {
		...
		case removeLocationUpdates:
			if (param.args.length > 0){
				int restrictState = getRestrictState(param, PrivacyManager.cLocation, "GMS.requestLocationUpdates");//if (isRestricted(param, PrivacyManager.cLocation, "GMS.requestLocationUpdates"))
				if (param.args[0] instanceof PendingIntent) {
					...
					case 1:
						synchronized (mMapProxy2) {
							if (mMapProxy2.containsKey(param.args[0]))
								param.args[0] = mMapProxy2.get(param.args[0]);
						}
					break;
			...
			if (param.args.length > 1)
				...
				case 1:
					if (param.args[1] instanceof PendingIntent)
						...
						InvocationHandler ih = new OnLocationChangedObfuscater(Binder.getCallingUid(), param.args[1]);
						Object proxy = Proxy.newProxyInstance(cl, new Class<?>[] { ll }, ih);

						// Use proxy
						synchronized (mMapProxy2) {
							mMapProxy2.put(param.args[1], proxy);
						}
						param.args[1] = proxy;
					}
